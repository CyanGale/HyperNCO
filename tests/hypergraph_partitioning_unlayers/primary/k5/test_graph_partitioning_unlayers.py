...
if __name__ == "__main__":
    ...
    for run_idx in range(1, 11):
        ...
        net = DirectProbModel(hg.num_v, 5).to(get_device())

        gini_cof_lambda = lambda e, n: (-800 + 0.25 * e) / 1000
        obj_cof_lambda = lambda e, n: e / 15000
        cons_cof_lambda = lambda e, n: e / 100 +e*0.01
        # cons_cof_lambda = lambda e, n: e / 100


        loss, outs, res = run_pubo(
            type="partitioning", net=net, X=torch.randn(1,1).to(get_device()),
            hypergraph=hg, num_epochs=10000, 
            lr=0.5, 
            opt='AdamW', 
