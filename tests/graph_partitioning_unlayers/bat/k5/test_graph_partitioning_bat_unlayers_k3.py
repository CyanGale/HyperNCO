...
if __name__ == "__main__":
    ...
    for run_idx in range(1, 21):
        set_seed(run_idx * 10)
        graph = from_file_to_graph(Datasets.Graph_bat.path, True, True).to(get_device())
        ...
        cons_cof_lambda = lambda e, n: e / 10000 +e*0.01

        loss, outs, res = run_qubo(
            type="partitioning", net=net, X=torch.randn(1,1).to(get_device()),
            graph=graph, num_epochs=5000, loss_func=loss_partitioning_onehot_qubo,
            lr=0.05, 
            opt='AdamW', 
