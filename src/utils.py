...
# def from_file_to_graph(file_path: str, reset_vertex_index: bool = False, remove_self_loops: bool = False):
#     from dhg import Graph
#     with open(file_path, "r") as file:
#         lines = file.readlines()
#         lines = [line for line in lines if line.strip() and not line.strip().startswith("#")]
#         lines = lines[1:]  # 跳过第一行 800 19176 ✅
#         edges = []
#         loops = []
#         for line in lines:
#             line = line.strip()
...
#         all_vertices = sorted(set(v for edge in edges for v in edge))
#         vertex_mapping = {old: new for new, old in enumerate(all_vertices)}
#         new_edges = [[vertex_mapping[u], vertex_mapping[v]] for u, v in edges]

#         print(f"INFO: {len(loops)} loops found")
#         print(f"✅ 真实边数: {len(edges)}")  # 这里会输出 19176！
#         print(f"✅ 真实节点数: {len(all_vertices)}")
