...
if __name__ == "__main__":
    ...
    for run_idx in range(1, 11):
        set_seed(run_idx * 10)
        graph = from_file_to_graph(Datasets.Graph_dblp.path, True, True).to(get_device())
        net = DirectProbModel(graph.num_v, 2).to(get_device())
        gini_cof_lambda = lambda e, n: (-380 + 0.25 * e) / 1000
