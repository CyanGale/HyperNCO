...
def run(net, X, graph, num_epochs, loss_fn, lr, opt, **kwargs):
    ...
    # file_handler = logging.FileHandler('/home/exs/work/k-grouping/tmp/收敛情况_maxcut.log')
    ...
    if opt =='rmsprop':
        ...
        if opt == "AdamW":
            optimizer = torch.optim.AdamW(net.parameters(), lr=lr, weight_decay=5e-8)
        else:
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        #     res = evaluate(outs[0], graph)
        #     logger.info(f"epoch:{epoch} | cuts:{res['cut_edges']} | converged:{outs[0].shape[0] - res['not_converged']}") # maxxcut
        #     # logger.info(f"epoch:{epoch} | gini_cof:{kwargs.get('gini_cof_lambda')(epoch, num_epochs)} | cuts:{res['cuts']} | converged:{outs[0].shape[0] - res['not_converged']}")
        #     # logger.info(f"epoch:{epoch} | gini_cof:{kwargs.get('gini_cons_cof_lambda')(epoch, num_epochs)} | correct_edges:{res['correct_edges']} | converged:{outs[0].shape[0] - res['not_converged']} | num_color:{res['num_color']}") 
        #########################################################
    del net, optimizer
    return loss, outs


def run_qubo(type: Literal["coloring", "partitioning", "maxcut"], net, X, graph, num_epochs, lr, opt, evaluate=False, loss_fn: Callable=None, **kwargs):#: Literal["Adam", "AdamW"] = "adamW"
    ...
    if type == "coloring":
        ...
        from .coloring.loss import loss_coloring_onehot_qubo as _loss_fn
        ...
        from .partitioning.loss import loss_partitioning_onehot_qubo as _loss_fn
    elif type == "maxcut":
        from .maxcut.utils import maxcut_construct_Q as construct_Q
        from .maxcut.utils import maxcut_evaluate as evaluate_f
        from .maxcut.loss import loss_maxcut_onehot_qubo as _loss_fn
    else:
    ...
    loss, outs = run(net, X, graph, num_epochs, lf, lr, opt, edge_index=edge_index, Q=Q, **kwargs)
    evalute_results = evaluate_f(outs[0], graph)
    if evaluate:
...
def run_pubo(
    type: Literal["coloring", "partitioning", "maxcut"], net, X, hypergraph, num_epochs, lr, opt: Literal["Adam", "AdamW"] = "adamW", evaluate=False, simple=False, **kwargs
):
